...
<div class="col-xs-12">
  <div class="container product-listing-cont">
    <div class="row no-padding product_responsive">
      ...
      <h3 class="section-heading" style="margin-left:20px; margin-top:10px;">Set Auto Re-Pricing</h3>
      <?php if (isset($errors)) { ?>
      ...
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="padding:0px !important;">
        <div class="prod-data-cont">
          <?php
                        ...
                        if ($allProducts) {  ?>
          <form name="pricingPdtsF" id="pricingPdtsF" method="post" action="<?php echo base_url('assign_pricing')?>" >
           <input type="hidden" id="pricing_rules_id" name="pricing_rules_id" value="<?php echo $this->uri->segment(2)?>" />
<div style="float:right;margin-bottom:50px;"> 
            <span style="margin-right:30px;">
              <input class="btn btn-cart" onClick="AllProducts('updateprices')"  type="button" name="updateprices_all" id="updateprices_all" value="Apply price rule on selected">
              </span>
              
              <span>
              <input class="btn btn-cart" onClick="AllProducts('delete')"  type="button" name="updateprices_all" id="updateprices_all" value="Remove price rule on selected">
              </span>
...
<table class="table table-hover table-responsive">
    <thead>
      <tr>
        ...
        <th width="7%">Product <br >Image</th>
        
          <th  width="49%">Product name<br>
<span style="font-size:12px; color:blue">ISBN/Detail</span></th>
            <th width="15%">Product <br> Type</th>       
       <th width="8%">Product <br> Condition</th>
         <th width="8%">Minimum<br>Amount </th>
          <th width="8%">  Maximum<br>Amount </th>
           <th width="5%"> Rule<br>Amount </th>
    ...
    <tbody>
			...
			$rule_defined = $this->comman_model->get("pricing_rules_products",array("seller_products_id" => $product['id'],"pricing_rules_id <> "=>$this->uri->segment(2)));
				if($rule_defined==0){	
    ...
    <div class="">
        <div>
            <h4 style="margin-bottom:0px; margin-top:0px !important;"><?php echo $product['title']?></h4>
            <strong style="color:blue"> Author:</strong><span style="color:blue"><?php echo $product['author']?></span></span> <br>
        ...
        <td>
        <?php
        $where = array("seller_products_id"=>$product['id'],"pricing_rules_id"=>$this->uri->segment(2));			
		//print_r($where);
		...
		if($already){
			//print_r($already);
			$stay_same = $already[0]["stay_same"];
			$stay_below_price = $already[0]["stay_below_price"];
			$stay_above_price = $already[0]["stay_above_price"];
			}
		echo $product["book_condition"];
		?>
        </td>
          <td> 
          <input type="text"  class="form-control"   size="3" name="stay_below_price[<?php echo $product['id']; ?>]" id="stay_below_price_<?php echo $product['id']; ?>" value="<?php echo $stay_below_price; ?>"  style="width:50px;" />
        ...
        <td> 
                  ...
                  <input type="text"  class="form-control"   size="3" name="stay_above_price[<?php echo $product['id']; ?>]" id="stay_above_price_<?php echo $product['id']; ?>" value="<?php echo $stay_above_price; ?>"  style="width:50px;" />
                  </span></td>
        ...
        <div style="color: #ff5b53; font-size: 16px; font-weight:bold;">
                  
                  
                  <span   id="min_new_price_<?php echo $product['id']; ?>">$<?php echo$product['price']; ?>
                  </span>&nbsp;&nbsp;</div>
          ...
          <td> 
                ...
                <?php /*?><a href="javascript:;" class="btn btn-cart"  onClick="UpdateProductPdtNew('<?php echo $product['id']; ?>','<?php echo $product['quantity']; ?>','<?php echo $product['price']; ?>')" >Update</a><?php */?> </td>
...
function AllProducts(action){
	...
	//$("#pricingPdtsF").attr("action"); //Will retrieve it
if(action =='update'){
$("#pricingPdtsF").attr("action", "<?php echo base_url("assign_pricing/update");?>"); //Will set it	
$("#pricingPdtsF").submit();
}else
if(action =='delete'){
	
	var r = confirm("Are you sure you want to delete pricing rules?");
			if (r == true) {
				
$("#pricingPdtsF").attr("action", "<?php echo base_url("assign_pricing/delete");?>");
$("#pricingPdtsF").submit();
			}
}else
if(action =='updateprices'){
	
	var r = confirm("Are you sure you want to update all price rules?");
			if (r == true) {
				
$("#pricingPdtsF").attr("action", "<?php echo base_url("assign_pricing/updateprices");?>");
$("#pricingPdtsF").submit();
			}
}
	}
