...
class Users extends CI_Controller
{
 ...
 function pay_file()
    ...
    //     // $seller = $this->comman_model::all();
    //     // $seller = $this->comman_model->get("user",array("user_id"=>$this->session->userdata('user_id')));
    //     $user = $this->comman_model->get("user",array("user_id"=>$this->session->userdata('user_id')));
    //     $this->_data['main_content'] = 'frontend/products/seller_update';
    //             $this->__loadView();
    // }
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($is_register) {
                    ...
                    if($this->input->post('user_type')=='seller'){
                        ...
                        $updated = $this->comman_model->save('store', $store_options);
                // print_r($store_options);
                    ...
                    {
                        {
                            $this->session->set_flashdata('success', 'Your account has been successfully created as a Buyer ! Please check your inbox to activate your account.');
                            redirect('registration');
                        }
                    }
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $login_details = array(
                    'email' => $this->input->post('email'),
                    'password' => md5($this->input->post('password')),
                    'user_type' => $this->input->post('Buyer'),
                    'role_id' => 3
                );
                ...
                if (!$user_data) {
                    ...
                    $this->session->set_userdata("users",$user_data[0]["first_name"]." ". $user_data[0]["last_name"]);
                   /* echo "<pre>";
                    echo $this->session->userdata($user_data[0]["first_name"]);
                    die;*/
                    $this->session->set_userdata('logged_in_as', 'i_am_buyer');
                    redirect('my_account');;
                }
            } else {
                $this->session->set_flashdata('errors', validation_errors());
    ...
    function Sell_login()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == true) {
                ...
                $login_details = array(
                    'email' => $this->input->post('email'),
                    'password' => md5($this->input->post('password')),
                    'user_type' => $this->input->post('Seller'),
                    'role_id' => 3
                );
                $user_data = $this->user_model->verify_user($login_details);
                if (!$user_data) {
                    ...
                    $this->session->set_userdata($user_data[0]);
                    $this->session->set_userdata("users",$user_data[0]["first_name"]." ". $user_data[0]["last_name"]);
                    $this->session->set_userdata('logged_in_as', 'i_am_seller');
                    redirect('my_account');;
                }
            } else {
                $this->session->set_flashdata('errors', validation_errors());
        ...
        $this->_data['main_content'] = 'frontend/pages/loginseller';
        $this->__loadView();
    }
    ...
    function security_Profile()
    {
        ...
        if ($this->session->userdata('logged_in') == TRUE) {
            ...
            $this->_data['main_content'] = 'frontend/pages/security_profile';
            $this->__loadView();
...
function forget_password()
    ...
    public function update_user_profile()
    {
        ...
        $user_id = $_POST['user_id'] ? $_POST['user_id']:''; //line_remember
        ...
        if($user_id){
            ...
            $isUpdated = $this->user_model->updateUserProfile($updated_data,array( 'user_id' => $this->session->userdata('user_id')));
    ...
    public function update_user_security_profile()
    {
...
$updated_data = array(
    ...
    $this->user_model->updateUserProfile(
                ...
                $saved = $this->comman_model->save('payment_method',
                    ...
                    $this->comman_model->update("store",array("user_id"=>$this->session->userdata('user_id')),$data_store);
    ...
    function addresses()
    {
       /* $data = $this->input->post();
        ...
        die();*/
            ...
            $this->_data['street_address2'] = $this->input->post("street_address2");      
