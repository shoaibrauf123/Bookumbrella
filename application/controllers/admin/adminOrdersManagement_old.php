...
//        check_is_login(6);
    ...
    function index()
    {
        ...
        $total = $this->purchase_model->get_total('purchase', $where,array('seller_id'=>''));
        $per_page = 10;
        $num_links = 4;
        $uri_segment = 3;

        $this->_data['pagination'] = paginate(base_url() . 'admin/orders/', $total, $per_page, $num_links, $uri_segment);
        $this->_data['results'] = $this->purchase_model->get_all_limited('purchase', $per_page, $start_from, $where,'id,user_id,SUM(qty) as qty,created_on,updated_on,seller_id,SUM(price) as price,order_status,payment_status,order_id',false,array('order_id'=>''));
...
//                          'buy_url' => $this->input->post('buy_url'),
 ...
 $updated = $this->comman_model->update('purchase', $where, $orderData);
                    ...
                    if ($updated) {
                        ...
                        $this->_data['errors'] = 'Order Updated successfully.';
                    }
